...
bot.onText(/\/start/, async (msg) => {
    ...
    bot.sendMessage(
        chatId,
        `Welcome to <b>BrokeCoin Volume Bot</b>\n\n` +
...
bot.onText(/\/help/, async (msg, match) => {
    const helpMessage = `
<b><u>Brokecoin Volume Bot - Help Menu</u></b>
...
<i>Developed with ❤️ by @SolanaSpecialOps</i>
`;
...
bot.on('callback_query', async (query) => {
  ...
  const taskId = query.data.replace('gather_', '');
  const task = await Task.findOne({ taskid: taskId });

  if (task.isRunning) {
    bot.sendMessage(task.userid, `❌ First stop the bot...`, { parse_mode: 'HTML' });
    return;
  }

  const user = await User.findOne({ userid: task.userid });
  const mainWallet = Keypair.fromSecretKey(new Uint8Array(user.private_key.split(',').map(Number)));
  let gathered = 0;

  for (const w of task.wallets) {
    try {
      const proxyWallet = Keypair.fromSecretKey(new Uint8Array(w.private_key.split(',').map(Number)));
      const balance = await connection.getBalance(proxyWallet.publicKey);

      if (balance > 0.002 * LAMPORTS_PER_SOL) {
        const lamportsToSend = balance - (0.001 * LAMPORTS_PER_SOL);
        ...
        const sig = await sendAndConfirmTransaction(connection, tx, [proxyWallet], {
          skipPreflight: true,
        });

        console.log(`✅ Gathered ${lamportsToSend / LAMPORTS_PER_SOL} SOL from ${proxyWallet.publicKey.toBase58()}`);
        gathered += lamportsToSend;
      } else {
        console.log(`⚠️ ${proxyWallet.publicKey.toBase58()} has insufficient funds.`);
      }
    } catch (err) {
      console.error(`❌ Failed to gather from one wallet:`, err.message);
    }
  }

  const sol = gathered / LAMPORTS_PER_SOL;
  if (sol > 0) {
    bot.sendMessage(user.userid, `✅ Successfully gathered ${sol.toFixed(4)} SOL from worker wallets.`, { parse_mode: 'HTML' });
  } else {
    bot.sendMessage(user.userid, `❌ No funds found in worker wallets.`, { parse_mode: 'HTML' });
  }

  refresh(taskId, query.message.message_id, query.message.chat.id);
}
